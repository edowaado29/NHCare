...
$obj = new Functions();
$selectAnakAsuh = $obj->get_data("SELECT * FROM tb_anakasuh");

if (isset($_POST['hapus'])) {
    $id_anakasuh = $_POST['hapus'];
    $result = $obj->delete_data("DELETE FROM tb_anakasuh WHERE id_anakasuh = '$id_anakasuh'");
    if ($result) {
                ...
                <div class="table-section">
                    <table>
                        ...
                        <tbody>
                            <?php 
                            $no = 1;
                            while($row = mysqli_fetch_assoc($selectAnakAsuh)) :
                            ?>
                            <tr>
                                <td><?php echo $no; ?></td>
                                <td><?php echo $row['id_anakasuh']; ?></td>
                                <td><?php echo $row['nama']; ?></td>
                                <td><?php echo $row['status']; ?></td>
                                <td>
                                <form action="" method="POST">
                                        <button type="button" onclick="window.location.href='detail_anak_asuh.php?id_anakasuh=<?php echo $row['id_anakasuh']; ?>'"><i class='bx bxs-show'></i></button>
                                        <button type="button" onclick="window.location.href='edit_anak_asuh.php?id_anakasuh=<?php echo $row['id_anakasuh']; ?>'"><i class='bx bxs-edit'></i></button>
                                        <button type="submit" name="hapus" value="<?php echo $row['id_anakasuh']; ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus anak asuh ini?');"><i class='bx bxs-trash'></i>
                                    </form>
                                </td>
                            </tr>
                            <?php 
