...
$obj = new Functions();
$selectAcara = $obj->get_data("SELECT * FROM tb_acara WHERE id_acara = $id_acara");
$rowAcara = mysqli_fetch_assoc($selectAcara);
...
if(isset($_POST['simpan'])){
    ...
    if(empty($judul) || empty($deskripsi) || empty($date)){
        $_SESSION['empty_form'] = true;
        header("Location: edit_acara.php?id_acara=$id");
    } else {
        $updateAcara = $obj->update_data("UPDATE tb_acara SET judul='$judul', deskripsi='$deskripsi', tanggal_acara='$date', id_user='$id_user' WHERE id_acara=$id");
        if($updateAcara){
            ...
            <div class= "home-body">
                <div class="card-body">
                    <form action="" method="POST">
                        <input type="hidden" name="id" value="<?php echo $_GET['id_acara']; ?>">
                        <div>
                            <label for="judul">Judul Acara</label>
                            <input type="text" id="judul" name="judul" value="<?php echo $rowAcara['judul']; ?>">
                        </div>
                        <div>
                            <label for="deskripsi">Deskripsi</label>
                            <textarea id="deskripsi" name="deskripsi" rows="5"><?php echo $rowAcara['deskripsi']; ?></textarea>
                        </div>
                        <div>
                            <label for="date">Tanggal dan Jam Acara</label>
                            <input type="datetime-local" id="date" name="date" value="<?php echo $rowAcara['tanggal_acara']; ?>">
                        </div>
                        <div class="btn-container">
                            <button type="button" onclick="window.location.href='acara.php'" class="btn btn-danger">Kembali</button>
