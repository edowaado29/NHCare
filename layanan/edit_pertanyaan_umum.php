...
$obj = new Functions();
$selectFaq = $obj->get_data("SELECT * FROM tb_faq WHERE id_faq = $id_faq");
$rowFaq = mysqli_fetch_assoc($selectFaq);
...
if(isset($_POST['simpan'])){
    ...
    if(empty($question) || empty($ask)){
        ...
        header("Location: edit_pertanyaan_umum.php?id_faq=$id");
    } else {
        $editFaq = $obj->update_data("UPDATE tb_faq SET pertanyaan = '$question', jawaban = '$ask' WHERE id_faq = $id");
        if($editFaq){
...
<head>
    ...
    <title>Edit Pertanyaan Umum</title>
...
<body>
    ...
    <section class="home-section">
        <div class="home-content">
            ...
            <h3>Edit Pertanyaan Umum</h3>
        </div>
            <div class= "home-body">
                <div class="card-body">
                    <form action="" method="POST">
                        <input type="hidden" name="id" value="<?php echo $_GET['id_faq']; ?>">
                        <div>
                            <label for="question">Pertanyaan</label>
                            <input type="text" id="question" name="question" value="<?php echo $rowFaq['pertanyaan']; ?>">
                        </div>
                        <div>
                            <label for="ask">Jawaban</label>
                            <textarea id="ask" name="ask" rows="5"><?php echo $rowFaq['jawaban']; ?></textarea>
                        </div>
